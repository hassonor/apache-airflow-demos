...
with DAG(dag_id='tz_dag', schedule_interval="0 1 * * *", default_args=default_args) as dag:
    ...
    """print('datetime from Python is Naive: {0}'.format(timezone.is_naive(datetime(2023, 9, 18))))
    print('datetime from Airflow is Aware: {0}'.format(timezone.is_naive(timezone.datetime(2023, 9, 18)) == False))
